...
router.get("/form/confer/:id", async (req, res) => {
  ...
  try {
    ...
    //   "SELECT full_page, published_journals, accepted, q_proof, call_for_paper, fee_receipt, fx_rate_document, conf_proof FROM "
    // )

    res.status(200).json({
      form: form[0],
      conf: conf[0],
      confername: infoConf[0].conf_name,
